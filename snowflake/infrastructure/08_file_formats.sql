...
CREATE OR REPLACE FILE FORMAT IDENTIFIER('LIGHTHOUSE_' || $env || '_RAW.OLTP.csv_format')
    TYPE = 'CSV'
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
    ...
    COMMENT = 'CSV format for OLTP CDC seed data — comma-delimited with header';

-- ─────────────────────────────────────────────────────────────────────────────
-- CRM schema — CSV format for SaaS connector seed files
-- ─────────────────────────────────────────────────────────────────────────────

CREATE OR REPLACE FILE FORMAT IDENTIFIER('LIGHTHOUSE_' || $env || '_RAW.CRM.csv_format')
    TYPE = 'CSV'
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
    FIELD_OPTIONALLY_ENCLOSED_BY = '"'
...
CREATE OR REPLACE FILE FORMAT IDENTIFIER('LIGHTHOUSE_' || $env || '_RAW.IOT.json_format')
    TYPE = 'JSON'
    STRIP_OUTER_ARRAY = TRUE
    STRIP_NULL_VALUES = FALSE
    COMMENT = 'JSON format for IoT telemetry event files — strips outer array';

-- ─────────────────────────────────────────────────────────────────────────────
-- Partner Feeds schema — CSV and Parquet formats
-- ─────────────────────────────────────────────────────────────────────────────

CREATE OR REPLACE FILE FORMAT IDENTIFIER('LIGHTHOUSE_' || $env || '_RAW.PARTNER_FEEDS.csv_format')
    TYPE = 'CSV'
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
    FIELD_OPTIONALLY_ENCLOSED_BY = '"'
    ...
    TRIM_SPACE = TRUE
    ERROR_ON_COLUMN_COUNT_MISMATCH = TRUE
    COMMENT = 'CSV format for partner feed files — grid usage, product catalog';

CREATE OR REPLACE FILE FORMAT IDENTIFIER('LIGHTHOUSE_' || $env || '_RAW.PARTNER_FEEDS.parquet_format')
    TYPE = 'PARQUET'
    COMPRESSION = 'SNAPPY'
    COMMENT = 'Parquet format for partner feed files — installation certifications';

-- ─────────────────────────────────────────────────────────────────────────────
-- Knowledge Base schema — CSV format for document metadata
-- ─────────────────────────────────────────────────────────────────────────────

CREATE OR REPLACE FILE FORMAT IDENTIFIER('LIGHTHOUSE_' || $env || '_RAW.KNOWLEDGE_BASE.csv_format')
    TYPE = 'CSV'
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
    FIELD_OPTIONALLY_ENCLOSED_BY = '"'
